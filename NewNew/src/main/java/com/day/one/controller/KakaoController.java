...
public class KakaoController {
	...
	private final static String K_CLIENT_ID = "aaa3f6f1d71ee47b9c8598762df25056";
	//private final static String K_REDIRECT_URI = "http://localhost:8080/login/oauth"; //개발용
	private final static String K_REDIRECT_URI = "http://13.125.237.11:8080/login/oauth"; //운영용
	 ...
	 * public JsonNode getKakaoUserInfo(String autorize_code) {
	 * 
	 * final String RequestUrl = "https://kapi.kakao.com/v1/user/me"; //String
	 * CLIENT_ID = K_CLIENT_ID; // REST API KEY //String REDIRECT_URI =
	 * K_REDIRECT_URI; // 리다이렉트 URI //String code = autorize_code; // 로그인 과정중 얻은 토큰
	 * 값 final HttpClient client = HttpClientBuilder.create().build(); final
	 * HttpPost post = new HttpPost(RequestUrl); String accessToken =
	 * getAccessToken(autorize_code); // add header post.addHeader("Authorization",
	 * "Bearer " + accessToken);
	 * 
	 * JsonNode returnNode = null;
	 * 
	 * try {
	 * 
	 * final HttpResponse response = client.execute(post); final int responseCode =
